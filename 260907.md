...
이제 플레이어가 공격하는 로직을 잡을 수 있다. 인터페이스를 써도 되고 안써도 되는 것은 자유.

Raycast를 통한 공격에서 방향은 여러 갈래인데 Tag를 붙일 것이냐 인터페이스냐의 차이. 인터페이스를 써보는 방향으로 해볼 것이다.
...
FPS 환경에서는 총에서는 나가게끔 하면 된다. 화면 가운데서 나가게끔 할 수 있다. 

그렇다보니까 카메라에 대한 것은 알아야 할 것이다.
```csharp
public class PlayerWeapon : MonoBehaviour
{
    // Raycast를 쏴서 IDamagable을 감지하는 것으로 사용.
    private Transform _cameraTransform;
}
```

지금까지 구현했던 것을 생각하면 인풋 여부 조건을 썼었다. 그런데 이게 직관적이지만 A키가 무슨 역할을 하는 것인지를 모른다. 입력에 대한 것을 KeyCode를 통해 구현해두고 bool 타입으로 눌렀는지에 대한 여부를 판단하도록 할 수 있다.
...
public class PlayerWeapon : MonoBehaviour
{
    ...
    public void Fire()
    {
        ...
        // Raycast를 쏘고 IDamagable을 받아오는 것. 이것은 함수화.

        // 받아오면 IDamagable에서 TakeDamage를 호출.
    }

    private IDamagable GetDamagable()
    {
...
public class PlayerWeapon : MonoBehaviour
{
    ...
    public void Fire()
    {
        ...
        // Raycast를 쏘고 IDamagable을 받아오는 것. 이것은 함수화.
        IDamagable damageble = GetDamagable();

        // 받아오면 IDamagable에서 TakeDamage를 호출.
        damageble.TakeDamage(_weaponDamage);
        // 이전 인터페이스에서 프로퍼티 값 적용.
        Debug.Log($"Player : {damageble.gameObject.name}에게 발사"); 
    }
...
```
WeaaponTransform에 대해 변경.

이제 임시적으로 Monster라는 스크립트
```csharp
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
그리고 발사를 GetKeyDown으로 하자.

큐브 쪽에 monster 넣도록 한다.
