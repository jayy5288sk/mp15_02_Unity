...
**델리게이트 선언한 클래스 내부에서만 발화할 수 있도록 만드는 것이 이벤트라는 개념이다.**

그런데 체력을 잘 받아오는 것은 알겠는데 델리게이트가 이것만 있으리라고는 보장할 수 없다.

그럼 그때마다 public delegate void FloatChange(float value); 등 줄줄히 추가를 해야 한다. 그러니까 타입이 하나씩 생긴다.

델리게이트는 직접 선언도 가능하지만 미리 저장하여 Template처럼 사용할 수 있는 기능이 있다.

## 3. Action, Function
### 3.1. 사용
반환값이 있을 때는 Function, 없을 때는 Action이다.
...
특정 이벤트에 대한 관리를 직접적으로 델리게이트를 선언하지 않고 다음과 같이 Action(혹은 Function)을 통해 선언한다.

![alt text](image-214.png)

당연히 매개변수를 여러개 담는 것도 가능하다.
...
그러면 델리게이트에 이벤트도 걸어주고 편하게 사용할 수 있다.

이제는 플레이어가 UI를 직접 알고 있는 상황에서 알고 있어야 하는 것이 많아졌던 것이 알아서 붙어서 동작을 하게 된다.

이렇게 되면 플레이어에서 만질 것이 없어진다.

플레이어의 status를 분산해서 가지고 있는 구조를 플레이어의 Status를 따로 관리하는 구조를 가지면 좀 더 좋은 구조이다.
...
팁이 있다면 사실상 UI마다 플레이어를 알고 있는 것도 불편한 구조이다.

UI는 갱신만의 책임이 필요할 것 같은데 단순히 자신만의 일을 책임할 수 있도록 하면 될 것 같다고 생각할 수도 있다.

구조적인 팁으로써는 다음과 같이 진행.
...
{
   public void RefreshGuage(int health)
    {
        Debug.Log("게이지 갱신");
    }
}
...
가정을 하나 들어서 Player가 파괴된거나 한다면 구독 관계를 끊어줘야 한다. 

AddListener를 통해 함수가 쌓이는데 파괴가 된다면 싹 다 없애줘야 한다.

그래서 델리게이트 자체를 Listner과의 관계를 끊어주기 위해
...
public class TempPlayer : MonoBehaviour
{
    ...
    public ObsevableProperty<float> Exp;
...
{
    public void RefeshGauge(float value)
    {
...
어떤 객체를 기준으로 걸 것인지, 찾아가서 그 기능을 호출하면 된다.
...
인스펙터에 걸어두고 싶으면 Unity 이벤트, 아니면 델리게이트인 셈.
...
또다른 함수를 역호출했는데 이런 구조를 콜백이라고 부른다.

그래서 갱신 조건에 대해 플레이어가 신경쓰는 것이 아니라 함수 여러개 등록해두고 간편히 갱신되게끔 하는 구조이다. 

그래서 이번에 해볼 수 있는 것은 UI 갱신하는 것을 델리게이트로 연습해보는 것을 한다.
...
메모리 주소가 복사되다보니 각 개별적인 것이 아닌 같은 것을 가리킨다. 

그래서 c2를 변경해도 c1이 변경된다.

이것이 이상하게 동작하는 참조 타입도 존재.
...
string이 정확히는 클래스로 구현된 참조 타입인데 불변성을 가진 참조 타입이다.

모든 참조타입들이 c들과 같이 동작되는 것이 아니다.

s1에다가 첫번째 문자열이 변할 수 없게 처리가 되고 s2에다가 새로 문자열을 넣어준 순간 원본 데이터가 다른 것으로 된다. 

그래서 원본 타입이 변할 수 없는 불변성이라는 것이 존재할 수 있다.

string을 쓸 경우, 값을 할당할때마다 객체가 생성된다. 

string은 변경될 때 새 객체가 생성되므로 특히나 **게임에서 최적화는 챙기지만 문자열을 많이 다뤄야 할 경우 StringBuilder라는 것을 차용하면 된다.**

어지간하면 문자열이 많이 변할 일은 없다. 
