...
- 플레이어 점프 구현 시, 땅에 붙어있는지 확인하는 것 중에 Raycast를 아주 작게 쏴서 밟고 있는 것이 땅인지 몬스터인지 확인하도록 하는 방법도 존재.
- 로직을 짤 때, 불러와지는지에 대해 Debug.Log를 통해 확인할 것.

## 2. 디버거
- 우측 하단 버그 아이콘을 눌러서 켜주고 IDE에서 Unity와 연결되도록 재생을 하여 아이콘이 파란색이 되면 중단점을 찍고 진행하면 된다. 
